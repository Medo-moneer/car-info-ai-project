...
# عنوان التطبيق
st.title("🚗 مشروع معلومات السيارات (AI Car Info)")
...
if not car_info.empty:
    ...
    if img_col:
        ...
        if pd.notna(image_url) and str(image_url).startswith("http"):
            ...
            st.info("📷 لا توجد صورة متوفرة لهذا الموديل.")
    else:
...
# === رسم بياني: توزيع السيارات حسب نوع الوقود ===
if "fuel_type" in df.columns:
...
if "year" in df.columns and "horsepower" in df.columns:
    ...
    ax.set_title("Year vs Horsepower (مقسّم حسب الوقود)")

    # إضافة وسيلة إيضاح للألوان
    if "fuel_type" in df.columns:
